...
from SCGM.parse import parse_mapping_table, parse_profile_for_piechart
...
class ParseTest(TestCase):
    ...
    def test_parse_profile_for_piechart_bad_filepath(self):
        self.assertRaises(IOError, parse_profile_for_piechart, "1111111")

    def test_parse_profile_for_piechart(self):
        filepath = "support_files/test_profile.txt"
        exp_data = [0.1336206897, 0.2740524781, 0.5923268322]
        exp_labels = ['o__Bacteroidales', 'o__Clostridiales', 'not_shared']

        obs_data, obs_labels = parse_profile_for_piechart(filepath)

        self.assertEqual(exp_data, obs_data)
        self.assertEqual(exp_labels, obs_labels)
